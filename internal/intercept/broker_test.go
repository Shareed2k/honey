...
// runningPodClient returns a fake clientset whose pod immediately reports
// every applied ephemeral container as running, so waitEphemeralRunning
// returns fast without needing to track the exact applied container name.
func runningPodClient(ns, pod string) *fake.Clientset {
	return runningPodsClient(ns, pod)
}
...
func newBrokerWith(t *testing.T, enf *policy.Enforcer, exec *recordingExecer, ttl time.Duration, clock func() time.Time) *Broker {
	t.Helper()
	cs := runningPodClient("prod-ns", "api-7d9f")
...
func TestBroker_Authorize_DeploysAndReturnsSession(t *testing.T) {
	...
	b := newBrokerWith(t, newAllowPolicy(t), exec, time.Hour, time.Now)
...
func TestBroker_Authorize_GateDeny(t *testing.T) {
	b := newBrokerWith(t, nil /*nil enforcer ⇒ fail-closed*/, &recordingExecer{}, time.Hour, time.Now)
...
func TestBroker_Authorize_PostDeployFailureSignalsAgent(t *testing.T) {
	...
	// The never-registered session must not linger in the registry.
	b.mu.Lock()
	defer b.mu.Unlock()
...
func TestBroker_Stop_WrongActorDenied(t *testing.T) {
	b := newBrokerWith(t, newAllowPolicy(t), &recordingExecer{}, time.Hour, time.Now)
...
func TestBroker_Janitor_StopsExpired(t *testing.T) {
	...
	b := newBrokerWith(t, newAllowPolicy(t), exec, time.Minute, clock)
	sess, err := b.Authorize(context.Background(), AuthorizeRequest{Actor: "alice", Cluster: "prod", Namespace: "prod-ns", Pod: "api-7d9f", Modes: local.Modes{Incoming: true}})
	require.NoError(t, err)
