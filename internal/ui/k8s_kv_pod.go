...
cleanup() {
  ...
  rm -f "/tmp/honey-kv-$$.py" 2>/dev/null || true
}
trap cleanup EXIT INT HUP TERM
printf %%s "$PYB64" | base64 -d > "/tmp/honey-kv-$$.py"
python3 "/tmp/honey-kv-$$.py" "$PORT" "$TOKEN" &
KV_PID=$!
i=0
...
sleep 0.05
exec sh -c "$(printf %%s "$INB64" | base64 -d)"
`, token, pyB64, cmdB64)

	outer := base64.StdEncoding.EncodeToString([]byte(bootstrap))
	return fmt.Sprintf(`printf %%s %s|base64 -d|exec sh`, shellSingleQuoted(outer)), nil
}
