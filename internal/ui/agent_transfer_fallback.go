...
with open('%s', 'rb') as f:
    ...
    with urllib.request.urlopen(req) as resp:
        pass
`, shellSingleQuoteEscape(srcPath), u.URL, size, string(headersJSON))
	return fmt.Sprintf("%s -c '%s'", pythonBin, shellSingleQuoteEscape(minifyPythonScript(code)))
}
...
`, shellSingleQuoteEscape(dstPath), u.URL, shellSingleQuoteEscape(dstPath))
	return fmt.Sprintf("%s -c '%s'", pythonBin, shellSingleQuoteEscape(minifyPythonScript(code)))
}
...
`, string(urlsJSON), partSize, shellSingleQuoteEscape(srcPath))
	return fmt.Sprintf("%s -c '%s'", pythonBin, shellSingleQuoteEscape(minifyPythonScript(code)))
}
