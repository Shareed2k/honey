...
func TestScriptRunner_RunLocalHost_Success(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(localScript, []byte("echo 'local script ran'"), 0o600)) // runner will chmod +x
...
func TestScriptRunner_RunLocalHost_Timeout(t *testing.T) {
	tmpDir := t.TempDir()
	localScript := filepath.Join(tmpDir, "test.sh")
	require.NoError(t, os.WriteFile(localScript, []byte("sleep 2"), 0o600))
...
func TestScriptRunner_Stream_DispatchesLocal(t *testing.T) {
	tmpDir := t.TempDir()
	localScript := filepath.Join(tmpDir, "test.sh")
	require.NoError(t, os.WriteFile(localScript, []byte("echo 'local'"), 0o600))
