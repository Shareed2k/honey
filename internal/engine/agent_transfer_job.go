...
func BuildAgentTransferJob(
	...
	if plan, ok, srcCap, dstCap := tryFallbackPlan(ctx, cache, resolvedSrcUser, resolvedDstUser, src, dst, srcPath, cloudNorm, transferCfg); ok {
		job.FallbackPlan = plan
		job.FallbackCapabilitySrc = srcCap
		job.FallbackCapabilityDst = dstCap
		return job, nil
	}
	...
	job.AgentLocalAbs = srcBin
	job.SourceAgentLocalAbs = srcBin
	job.DestAgentLocalAbs = dstBin
	return job, nil
}

// tryFallbackPlan returns a presign.Plan for the curl path, or (nil, false) when the
...
func tryFallbackPlan(
...
) (*presign.Plan, bool, string, string) {
	if cfg.ForceAgentPath {
		zap.L().Debug("tryFallbackPlan: skipping because ForceAgentPath is true")
		return nil, false, "", ""
	}
	...
	srcCap, srcErr := detectFallbackCapabilityViaRunner(runner, srcKey)
	if srcErr != nil || srcCap == "" {
		zap.L().Debug("tryFallbackPlan: skipping because source host lacks fallback capability", zap.Error(srcErr), zap.String("cap", srcCap))
		return nil, false, "", ""
	}
	dstCap, dstErr := detectFallbackCapabilityViaRunner(runner, dstKey)
	if dstErr != nil || dstCap == "" {
		zap.L().Debug("tryFallbackPlan: skipping because destination host lacks fallback capability", zap.Error(dstErr), zap.String("cap", dstCap))
		return nil, false, "", ""
	}
	...
	if err != nil {
		zap.L().Debug("tryFallbackPlan: skipping because stat failed", zap.Error(err))
		return nil, false, "", ""
	}
	var size int64
	if _, err := fmt.Sscanf(strings.TrimSpace(sizeOut), "%d", &size); err != nil {
		zap.L().Debug("tryFallbackPlan: skipping because size parse failed", zap.String("output", sizeOut), zap.Error(err))
		return nil, false, "", ""
	}
	if size > cfg.PresignedMaxSizeBytes {
		zap.L().Debug("tryFallbackPlan: skipping because file too large", zap.Int64("size", size), zap.Int64("max", cfg.PresignedMaxSizeBytes))
		return nil, false, "", ""
	}
	...
	if err != nil {
		zap.L().Debug("tryFallbackPlan: PlanTransfer failed", zap.Error(err))
		return nil, false, "", ""
	}
...
// attempts (the original fallback-path attempt and the agent-path retry).
// RunAgentTransferWithFallback ...
func RunAgentTransferWithFallback(
	...
	if execErr != nil && job.FallbackPlan != nil && transferCfg.PresignedRetryWithAgent {
