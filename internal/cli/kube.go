...
const kubeDeviceEnrollPath = "/api/v1/devices/enroll"
...
var kubeLoginCmd = &cobra.Command{
	...
	Long: `Redeems a one-time device enrollment code (minted by an operator via
honey device enroll-code) for a short-lived client certificate signed by
honey's device CA, then writes a kubeconfig cluster/user/context that points
kubectl at the honey Kubernetes access proxy for the given cluster.
...
func init() {
	kubeLoginCmd.Flags().StringVar(&kubeLoginEnrollCode, "enroll-code", "", "One-time enrollment code from honey device enroll-code (required; fixes the certificate identity)")
...
func runKubeLogin(cmd *cobra.Command, args []string) error {
	...
	if code == "" {
		return fmt.Errorf("--enroll-code is required")
	}
	...
	switch {
	case proxyCAPath == "" && !kubeLoginInsecure:
	...
	if proxyCAPath != "" {
		var err error
		proxyCA, err = safepath.ReadFile(proxyCAPath)
		if err != nil {
	...
	merged := mergeKubeContext(existing, kubeContextOpts{
		...
		keyPEM:                keyPEM,
		caPEM:                 proxyCA,
		insecureSkipTLSVerify: kubeLoginInsecure,
