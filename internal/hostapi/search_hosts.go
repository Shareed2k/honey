...
type SearchHostsInput struct {
	...
	DockerDiscoverProviders string `json:"docker_discover_providers,omitempty"`
...
func SearchHosts(ctx context.Context, in *SearchHostsInput) (SearchHostsOutput, error) {
	...
	q := hosts.Query{
		...
		DockerDiscoverProviders: hosts.ParseProviders(strings.TrimSpace(in.DockerDiscoverProviders)),
	...
	pf := searchrun.ProviderFlags{
		...
		DockerSocket:            strings.TrimSpace(in.DockerSocket),
		DockerPlatform:          strings.TrimSpace(in.DockerPlatform),
		DockerDiscoverProviders: strings.TrimSpace(in.DockerDiscoverProviders),
	}
