...
func (w *StepWrapper) UnmarshalJSON(data []byte) error {
	...
	for _, e := range stepRegistry {
		for _, key := range e.actionKeys {
			...
			if matchedKind != "" && matchedKind != e.kind {
				return fmt.Errorf("only one of command, render, put, get, script, agent_transfer, ai, template, plugin, tunnel, k8s, docker, opensearch, postgres, recipe, opa, http, or intercept allowed")
			}
			matchedKind = e.kind
			ctor = e.ctor
		}
	...
	if ctor == nil {
		return fmt.Errorf("need exactly one of command, render, put, get, script, agent_transfer, ai, template, plugin, tunnel, k8s, docker, opensearch, postgres, recipe, opa, http, or intercept")
	}
