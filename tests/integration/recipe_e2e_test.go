...
func TestRecipeE2E_DockerStep(t *testing.T) {
	...
	// Target the DinD daemon explicitly via the step socket field (the SDK
	// executor uses client.FromEnv for local records, so CLIEnv is not enough).
	cue := fmt.Sprintf(`
recipe: {
	name: "test-docker-step"
	type: "linear"
	steps: [
		{ host: "*", docker: { socket: %q, action: "run", run: { image: "alpine", command: ["echo", "dind-works"] } } },
	]
}
`, dindHost)
	...
	require.Len(t, results, 1)
	assert.True(t, results[0].Success, "run failed: %s", results[0].ErrMsg)
	assert.Contains(t, results[0].Output, "dind-works")
}

func TestRecipeE2E_K8sStep(t *testing.T) {
	rec := k8sTestRecord(t) // wires Meta[kubeconfig], which newK8sClients reads
	reg := &testRegistry{}
...
data:
  key: value
""" } } },
		{ host: "*", k8s: { get: { resource: "configmap/e2e-config" } } },
	]
}
`
	...
	assert.True(t, results[1].Success, "get failed: %s", results[1].ErrMsg)
	assert.Contains(t, results[1].Output, "e2e-config")
}
