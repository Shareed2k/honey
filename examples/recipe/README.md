...
- `kv_tunnel_multistep_example.cue`: Three **`command`** steps with **`defaults.kv_tunnel: true`** — the same operator KV store spans all steps on SSH (see `honey cue-exec` docs); each host uses a key `mstep_$HONEY_HOST_NAME` so parallel hosts do not overwrite each other.
