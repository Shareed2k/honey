...
- `intercept.cue`: **Graph** recipe with a targetless **`intercept`** step (egress + env into a cluster namespace, no target pod, no SSH) and a follow-up step reusing the same agent via **`session_step`**; see [CUE Recipes — Intercept steps](https://github.com/shareed2k/honey/blob/main/website/docs/cue-recipes.md#intercept-steps) and [Local Interception](https://github.com/shareed2k/honey/blob/main/website/docs/intercept.md).
...
- `prometheus_query_demo.cue`: **`plugin:`** prometheus WASM PromQL instant query (official `client_golang` API client over the WASM HTTP bridge); server URL + auth live in `plugins/prometheus/plugin.yaml` (`config.prometheus_url`, `PROMETHEUS_BEARER_TOKEN`), not the recipe; see file header for `make build-plugin-modules`.
- `prometheus_health_check.cue`: **Graph** recipe — three parallel prometheus WASM queries (targets up, scrape duration, series count) → **`ai`** summary; uses only Prometheus's own self-scrape metrics, so it runs against a bare `docker run prom/prometheus`.
